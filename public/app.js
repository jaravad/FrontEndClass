...
db.collection("users").onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        if (doc.data().state == true) {//Revisa el estado para poner el boton habilitado o deshabilitado
            tabla.innerHTML += `
            <article class="person-card col-4 col-sm-3 col-md-2 col-lg-1 d-flex flex-column align-items-center">
            <img class="w-75" src="./images/faces/9.jpg" alt="Profile photo" />
            <p>${doc.data().first}</p>
            <p>${doc.data().last}</p>
            <p>${doc.data().born}</p>
            <a href="" type="button" class="btn btn-danger my-1" onclick=eliminar('${doc.id}')>Delete</a>
            <a href="" type="button" class="btn btn-primary my-1" data-toggle="modal" data-target="#modalRegisterForm" onclick=update('${doc.id}','${doc.data().first}','${doc.data().last}','${doc.data().born}')>Update</button>
            <a href="" type="button" class="btn btn-success my-1" onclick=habilitar('${doc.id}','${doc.data().first}','${doc.data().last}','${doc.data().born}')>Habilitado</a>
            </article>`;
        } else {
            tabla.innerHTML += `
            <article class="person-card col-4 col-sm-3 col-md-2 col-lg-1 d-flex flex-column align-items-center">
            ...
            <a href="" type="button" class="btn btn-danger my-1" onclick=eliminar('${doc.id}')>Delete</a>
            <a href="" type="button" class="btn btn-primary my-1" data-toggle="modal" data-target="#modalRegisterForm" onclick=update('${doc.id}','${doc.data().first}','${doc.data().last}','${doc.data().born}')>Update</button>
            <a href="" type="button" class="btn btn-warning my-1" onclick=habilitar('${doc.id}','${doc.data().first}','${doc.data().last}','${doc.data().born}')>Deshabilitado</a>
            </article>`;
        }
